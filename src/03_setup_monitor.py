...
if  __name__ == "__main__":
    ...
    logger.info(f"Preprocesing the data from {app_infernece_table_name} for monitoring.")
    app_inference_processed_df = prepare_data_to_monitor(app_infernece_table_name)

    logger.info(f"Writing the prperocessed data to the table {app_inference_processed_table_name}")
    create_processed_table_if_not_exists(app_inference_processed_table_name, app_inference_processed_df, checkpoint_location)

    logger.info(f"Setting up the Lakehouse Monitoring from the {app_inference_processed_table_name}")
