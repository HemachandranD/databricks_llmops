...
def write_data_to_delta(df, mode, external_path, table_name):
    try:
        ...
        if external_path is not None:
            ...
            ).option("path", external_path).saveAsTable(table_name)
...
def write_embedding_data_handler(df_embed, embeddings_fqn):
    ...
    try:
        if not (spark.catalog.tableExists(embeddings_fqn)):
            ...
            spark.sql(
                spark.sql(
                    f"""CREATE TABLE IF NOT EXISTS {embed_table_name}(
        ...
        write_data_with_cdc(
            df_embed, mode="append", external_path=None, table_name=embeddings_fqn
        )
...
def set_alias(model_name, current_model_version):
    ...
    mlflow.tracking.MlflowClient().set_registered_model_alias(
        name=model_name, alias="champion", version=current_model_version
    )
...
def unpack_requests(
    ...
    requests_timestamped = requests_raw.withColumn(
        "timestamp", (col("timestamp_ms") / 1000)
    ).drop("timestamp_ms")
    ...
    # Unpack JSON.
    requests_unpacked = requests_success.withColumn(
        "request",
        F.from_json(F.expr(f"request:{input_request_json_path}"), input_json_path_type),
    ).withColumn(
        "response",
        F.from_json(
            F.expr(f"response:{output_request_json_path}"), output_json_path_type
        ),
    )

    if keep_last_question_only:
        requests_unpacked = requests_unpacked.withColumn(
            "request", F.array(F.element_at(F.col("request"), -1))
        )

    # Explode batched requests into individual rows.
    requests_exploded = requests_unpacked.withColumn(
        "__db_request_response",
        F.explode(
            F.arrays_zip(col("request").alias("input"), col("response").alias("output"))
        ),
    ).selectExpr(
        "* except(__db_request_response, request, response, request_metadata)",
        "__db_request_response.*",
    )
