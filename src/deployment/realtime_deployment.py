...
def real_time_deploy(logger, serving_endpoint_name, latest_model_version, endpoint_config):
    ...
    if existing_endpoint == None:
        logger.info(f"Creating the endpoint {serving_endpoint_url}, this will take a few minutes to package and deploy the endpoint...")
        w.serving_endpoints.create_and_wait(name=serving_endpoint_name, config=endpoint_config)

    # If endpoint does exist, update it to serve the new version
    else:
        logger.info(f"Updating the endpoint {serving_endpoint_url} to version {latest_model_version}, this will take a few minutes to package and deploy the endpoint...")
