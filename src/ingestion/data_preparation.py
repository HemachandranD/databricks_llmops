...
from src.common.utility_functions import write_data_to_delta
...
def process_pdf_files(logger, datasets_path, catalog_name, schema_name, raw_table_name):
    ...
    write_data_to_delta(
        df=raw_df, mode="overwrite", external_path=None, table_name=table_name
    )
    logger.info(
        f"Writing the raw data from pdf to the delta table {table_name} is completed."
    )
