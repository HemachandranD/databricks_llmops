...
from src.preprocessing.vector_indexing import self_managed_vector_index, managed_vector_index
...
from src.config.configuration import datasets_path, catalog_name, bronze_schema_name, silver_schema_name, gold_schema_name, pdf_raw_table_name, pdf_chunk_size, pdf_chunks_table_name, pdf_embeddings_table_name, vector_search_endpoint_sub_name, pdf_self_managed_vector_index_name, pdf_managed_vector_index_name
...
if __name__ == "__main__":
    ...
    logger.info(f"Writing the Chunked data to the delta table {catalog_name}.{silver_schema_name}.{pdf_chunks_table_name}")
    ...
    if (vector_client.list_endpoints()=='') or (vs_endpoint_name not in [e["name"] for e in vector_client.list_endpoints().get("endpoints", [])]):
    ...
    logger.info(f"Creating the index {vs_index_fullname} on vector serach endpoint {vs_endpoint_name}")
